...
export const SortedObjectMarshalUnit = defineMarshalUnit(
  (value: any, { marshal }) => {
    if (typeof value !== 'object' || !value) return pass;
    if (typeof value.length === 'number') return pass; // assume it's some other form of fancy array
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      changed = changed || keys[i] !== sortedKeys[i];
