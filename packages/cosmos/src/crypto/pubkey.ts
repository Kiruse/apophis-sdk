...
const encodings: Record<string, Record<string, (value: PublicKey) => unknown>> = {
  protobuf: {
    secp256k1: (value: PublicKey) => Any(
      ...
      Secp256k1PublicKey.encode(Secp256k1PublicKey.fromPartial({ key: value.bytes })).finish()
    ),
    ed25519: (value: PublicKey) => Any(
      CosmosEd25519TypeUrl,
      Ed25519PublicKey.encode(Ed25519PublicKey.fromPartial({ key: value.bytes })).finish()
    ),
  },
  amino: {
    ...
    decode(network: NetworkConfig, encoding: string, value: unknown) {
      ...
      switch (encoding) {
        case 'protobuf': {
          ...
          switch (type) {
            case 'secp256k1': {
              const key = Secp256k1PublicKey.decode(value.value);
              return new PublicKey(type, key.key);
            }
            case 'ed25519': {
              const key = Ed25519PublicKey.decode(value.value);
              return new PublicKey(type, key.key);
            }
          }
        }
